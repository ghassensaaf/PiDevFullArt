...
public class forgot_passController implements Initializable {
    ...
    void send_email(ActionEvent event) throws Exception {
        ...
        email.sendEmail(tfemail.getText(), "Réinitialisation de votre mot de passe", " Nous avons bien reçu votre demande de réinitialisation de votre mot de pass.  Veuillez saisir ce code dans la platforme " + String.valueOf(rand_int1));
...
//                controller.initData(txtlogin.getText());
    ...
    @FXML
    void Action_retour_login(ActionEvent event) {
        try {
            FXMLLoader loader=new FXMLLoader();
            loader.setLocation(getClass().getResource("/fxml/login.fxml"));
            Parent p=loader.load();
